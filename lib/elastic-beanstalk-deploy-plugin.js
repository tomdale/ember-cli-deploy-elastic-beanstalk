...
module.exports = DeployPlugin.extend({
  ...
  willUpload: function(context){
    ...
    var zip = new RSVP.Promise(function(resolve, reject) {
      var proc = childProcess.spawn('zip', ['-r', zipPath, dir]);

      var log = function(data) {
        self.log(data, { verbose: true });
      };
    ...
    //return exec("zip -q -r " + zipPath + " *", {
      //cwd: path.dirname(dir)
    //})
    return zip.then(function(){
        var zipStream = fs.createReadStream(zipPath);
        return md5Hash(zipStream);
      })
      .then(function(hash) {
        self.log("calculated zip hash " + hash, { verbose: true });
